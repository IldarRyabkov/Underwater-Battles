...
class Room:
    ...
    def setup_new_mobs(self, new_mobs):
        ...
        if self.new_mobs and self.new_mobs[0].name == 'BossSkeleton':
            self.boss_in_room = True
        else:
            self.boss_in_room = False
    ...
    def game_is_over(self):
        return self.boss_in_room and len(self.mobs) == 1
    ...
    def draw_mobs(self, surface, dx, dy):
        start = 1 if self.boss_in_room else 0
        for i in range(start, len(self.mobs)):
            if self.mobs[i].body_rect.colliderect(self.screen_rect):
                self.mobs[i].body.draw(surface, dx, dy)

    def draw_new_mobs(self, surface, dx, dy):
        start = 1 if self.boss_in_room else 0
        for i in range(start, len(self.new_mobs)):
            if self.new_mobs[i].body_rect.colliderect(self.screen_rect):
                self.new_mobs[i].body.draw(surface, dx, dy)

    def draw_boss_skeleton(self, surface, dx, dy, new_room=False):
        if new_room:
            self.new_mobs[0].body.draw(surface, dx, dy)
        else:
            self.mobs[0].body.draw(surface, dx, dy)
